...
- **La pantalla final** (`mostrar_pantalla_final`): `ax.text(...)` dibuja el resumen de la partida sobre `gameover.jpg`, con `path_effects.Stroke` para el contorno negro que lo hace legible sobre la foto. El panel de gráficos de la Sección 9 (`dibujar_panel_resultados`), en cambio, usa `fig.suptitle` con el mismo truco de `path_effects.Stroke` pero sobre fondo blanco: probamos ponerlo sobre `gameover.jpg` como la pantalla final, pero con 4 gráficos densos encima (uno de ellos con texto en cada celda) la foto de fondo terminaba estorbando en vez de ayudar, así que se sacó y quedó solo el título en texto.
